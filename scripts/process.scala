...
for( day <- 1 to 31){
	...
	rdd.filter(row => row.getInt("created_at_month").equals(month)&&row.getInt("created_at_day").equals(day)&&row.getInt("created_at_year").equals(year)).map
			{

				 => row.getSet[String]("hashtags")

			}.collect.foreach(set => set.map
				{
	...
	rrdout.saveToCassandra("trendcount","tes2", SomeColumns("created_at_year","created_at_month","created_at_day","mystring"))
	}
}
